...
pub async fn create_table(connection: &DatabaseConnection) {
    sqlx::raw_sql(
        "CREATE TABLE IF NOT EXISTS waitlist(id SERIAL PRIMARY KEY, account TEXT NOT NULL, ip TEXT UNIQUE NOT NULL)",
    )
    .execute(connection)
    .await
...
impl DatabaseWaitlist {
    ...
    pub async fn get_all(database: &Database) -> Result<Vec<Self>, Error> {
        query_as("SELECT id, account, ip FROM waitlist")
            .fetch_all(&database.connection)
            .await
    }

    pub async fn get_by_account(database: &Database, account: &str) -> Result<Option<Self>, Error> {
        query_as("SELECT id, account, ip FROM waitlist WHERE account = $1")
            .bind(account)
    ...
    pub async fn get_by_ip(database: &Database, ip: &str) -> Result<Option<Self>, Error> {
        query_as("SELECT id, account, ip FROM waitlist WHERE ip = $1")
            .bind(ip)
            .fetch_optional(&database.connection)
            .await
    }
    ...
    pub async fn insert(&mut self, database: &Database) -> Result<(), Error> {
        let id: i32 =
            query_scalar("INSERT INTO waitlist(account, ip) VALUES ($1, $2) RETURNING id")
                .bind(&self.account)
                .bind(&self.ip)
